        ...
        for i in xrange(self.num_steps):
            ...
            if not adv_windowed_list:
                self.global_step,  adv_windowed_list = self.g_model.train_step(
                    batch, adv_windowed_list, discriminator=(self.d_model if c.ADVERSARIAL else None))

            start = True
            if (sum(adv_windowed_list) / len(adv_windowed_list)<= delta / c.WINDOW_SIZE):
                self.global_step,  adv_windowed_list = self.g_model.train_step(
                        batch, adv_windowed_list, discriminator=(self.d_model if c.ADVERSARIAL else None))
            
            counter = 0  
            while(sum(adv_windowed_list) / len(adv_windowed_list) > delta / c.WINDOW_SIZE):
                print (sum(adv_windowed_list) / len(adv_windowed_list) , delta / c.WINDOW_SIZE)
                if not start:
...
def usage():
    ...
    print '-w/--window_size=  <Window size of online gradient descent'
...
def main():
    ...
    try:
        opts, _ = getopt.getopt(sys.argv[1:], 'l:t:r:a:n:s:OTH',
                                ['load_path=', 'test_dir=', 'recursions=', 'adversarial=', 'window_size=', 
                                 'name=', 'steps=', 'overwrite', 'test_only', 'help', 'stats_freq=',
